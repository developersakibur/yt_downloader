...
async function loadDownloadedFiles() {
  const filesList = document.getElementById("downloaded-files");
  filesList.innerHTML = ""; // Clear existing files

  try {
    const folders = await api("/downloads");
    if (folders && folders.length > 0) {
      folders.forEach(folder => {
        const folderItem = document.createElement("li");
        folderItem.textContent = folder.name;
        folderItem.classList.add("folder-item"); // Add a class for styling
        const sublist = document.createElement("ul");
        sublist.classList.add("file-sublist"); // Add a class for styling
      ...
      filesList.innerHTML = "<li>No matching folders found.</li>";
    }
  } catch (error) {
